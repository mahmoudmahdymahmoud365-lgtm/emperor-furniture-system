// ==============================
// Users Route — API-first auth with bcrypt + rate limiting + session tokens
// ==============================
const router = require("express").Router();
const pool = require("../db");
const bcrypt = require("bcryptjs");
const { createRateLimiter } = require("../middleware/rateLimiter");
const { createSession, destroySession } = require("../middleware/sessionAuth");
...
// ==============================
// POST /users/login — Authenticate + issue session token
// ==============================
router.post("/login", loginLimiter, async (req, res, next) => {
  try {
    ...
    res.json({
      ...toApi(user),
      sessionToken,
      sessionExpiresIn: 8 * 60 * 60 * 1000, // 8h in ms
    });
  } catch (e) { next(e); }
});
...
router.get("/session", (req, res) => {
  ...
  const authHeader = req.headers.authorization;
  if (!authHeader || !authHeader.startsWith("Bearer ")) {
    return res.status(401).json({ valid: false, code: "SESSION_INVALID" });
  }
  const session = validateSession(authHeader.slice(7));
  if (!session) {
    return res.status(401).json({ valid: false, code: "SESSION_EXPIRED" });
  }
  res.json({ valid: true, userId: session.userId, email: session.email, role: session.role });
});
